...
namespace NuiN.NExtensions
{
    ...
    public class ObjectPool<T> : IDisposable where T : MonoBehaviour, IPoolabeObject<T>
    {
        Stack<T> _stack = new();
        Func<T> _create;
        Action<T> _onGet;
        ...
        public ObjectPool(Func<T> create, Action<T> onGet = null, Action<T> onRelease = null, int preloadCount = 0, bool autoEnableDisable = true)
        {
            _onGet = onGet;
